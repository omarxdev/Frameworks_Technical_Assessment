...
import { formatDate, formatDateRange } from "@/features/portal/lib/format";
...
const BlockerRow = ({ blocker }: { blocker: AvailabilityBlocker }) => {
  ...
  return (
    <li className="flex gap-2.5 rounded-lg bg-stop-surface px-3 py-2 text-stop-foreground">
      ...
      return (
        ...
        >
          ...
          {blockers.length === 0 && asset.availability.verificationStale && (
            <p className="rounded-lg bg-warn-surface px-3 py-2 text-sm text-warn-foreground">
              Free for your dates, but our verification is more than 30 days
              old. We will re-confirm with the media owner before any contract
              is issued — it is a caution, not a refusal.
            </p>
          )}
