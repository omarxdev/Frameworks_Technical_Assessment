...
const blockersForAsset = (params: {
  ...
  const holdBlockers = holds
    ...
    .map<AvailabilityBlocker>((h) => ({
      kind: "hold",
      id: h.id,
      label: `Active hold until ${h.expiresAt}`,
...
export const evaluateExclusiveAssetAvailability = (params: {
  ...
  const assetOptions = productAssets.map<AssetOption>((asset) => {
    ...
    const reason = blockers.length
      ...
      : stale
        ? asset.note?.trim() ||
          `Last verified ${asset.verifiedAt ?? "never"}; confirm with the media owner before approval.`
  ...
  const summary: AvailabilitySummary = productAssets.length === 0
    ? {
        state: "unavailable",
    ...
    : free.length > 0
      ? {
          state: "available",
          reason: `${free.length} of ${productAssets.length} assets free for the requested dates.`,
          calculatedAt: FIXTURE_CLOCK,
      ...
      : needsConfirmation.length > 0
        ? {
            ...
            reason: `${needsConfirmation.length} asset(s) are free but their verification is over ${VERIFICATION_STALE_AFTER_DAYS} days old. Confirm with the media owner before approval.`,
            calculatedAt: FIXTURE_CLOCK,
        ...
        : {
            state: "unavailable",
            reason: `All ${productAssets.length} assets are booked, on active hold, or under a confirmed outage for the requested dates.`,
            calculatedAt: FIXTURE_CLOCK,
            availableAssetCount: 0,
            freshestVerificationAt: null,
            verificationStale: false,
